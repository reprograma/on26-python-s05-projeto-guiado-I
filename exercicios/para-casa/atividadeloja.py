...
#o usuario insere o id do produto desejado, o for in determina que a busca seja feita na lista de produtos, e caso esteja la ele deve ser exibido
def detalhes_produto():
    exibir_produto = input(int("Determine o id do produto desejado"))
    for index in lista_produtos:
        if exibir_produto == index.get("id"):
            print(f"index")



# criei a função apagar_produto, seguindo o padrão da professora. O usuário deve inserir o id desejado.  criei o for in range(len()), para determinar que percorresse a lista dos produtos do inicio ao fim. O 1o if foi criado para que em determinado momento essa busca fosse sessada. O 2o if é pra o id seja identificado dentro da lista e "escolhido". O .pop(index) indica que o item será removido
def apagar_produto():
    id_produto = int(input("Determine o produto a ser apagado:"))
    for index in range(len(lista_produtos)):
            if index < len(lista_produtos):
                if id_produto == lista_produtos[index].get("id"):
                    lista_produtos.pop(index)
                    print(f"Produto:  {id_produto} foi excluído")
